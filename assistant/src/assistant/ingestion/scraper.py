...
def _get_robots_parser(cache: Dict[str, RobotFileParser], url: str) -> RobotFileParser:
    """Return a cached robots parser for the base domain of a URL."""
    parsed = urlparse(url)
...
def _can_fetch(cache: Dict[str, RobotFileParser], url: str) -> bool:
    """Check robots.txt to determine whether the URL may be fetched."""
...
def _clean_html(html: str) -> str:
    """Remove navigation, media and noisy markup from HTML before markdown conversion."""
...
def _extract_title(html: str) -> str:
    """Extract a human-readable title from HTML, preferring an H1 header."""
    soup = BeautifulSoup(html, "html.parser")
    h1 = soup.find("h1")
    if h1:
...
def _slugify_url(url: str) -> str:
    """Create a filesystem-safe slug from a URL path."""
    parsed = urlparse(url)
...
def _hash_text(text: str) -> str:
    """Return a SHA256 hash of the supplied text."""
    return hashlib.sha256(text.encode("utf-8")).hexdigest()


def _relpath(path: Path) -> str:
    """Return a repository-relative path string if possible."""
    try:
...
def _build_metadata(seed: SeedUrl, url: str, title: str, markdown: str, output_path: Path) -> dict:
    """Build metadata for a scraped document before saving it."""
    parsed = urlparse(url)
    return {
        "source_url": url,
        "title": title,
...
def _save_markdown(output_path: Path, metadata: dict, markdown: str) -> None:
    """Write markdown content together with frontmatter metadata to disk."""
    content = "---\n" + json.dumps(metadata, ensure_ascii=False, indent=2) + "\n---\n\n" + markdown
    output_path.write_text(content, encoding="utf-8")


def _save_raw_html(raw_path: Path, html: str) -> None:
    """Persist the raw HTML response for later inspection."""
    raw_path.write_text(html, encoding="utf-8")


def _output_paths(seed: SeedUrl, url: str) -> tuple[Path, Path]:
    """Create output file paths for saved markdown and optional raw HTML."""
...
def _scrape_one(client: httpx.Client, cache: Dict[str, RobotFileParser], seed: SeedUrl) -> Optional[Path]:
    """Scrape a single seed URL and return the saved markdown path if successful."""
    url = seed.url
...
def scrape(seed_urls: Iterable[SeedUrl]) -> List[Path]:
    """Scrape all provided seed URLs and return the list of saved markdown file paths."""
...
def load_seed_urls(path: Path) -> List[SeedUrl]:
    """Load seed URLs from a JSONL file or fallback to built-in defaults."""
    if not path.exists():
        return SEED_URLS
