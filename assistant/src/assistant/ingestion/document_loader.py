...
def load_documents(root_dir: Path) -> List[Document]:
    """Load all supported documents under a root directory into Document objects."""
...
def _load_document(path: Path) -> Iterable[Document]:
    """Load a single file and return zero or more Document objects."""
...
def _parse_frontmatter(text: str) -> Tuple[str, dict]:
    """Extract JSON frontmatter from Markdown files, returning content and metadata."""
...
def _normalize_metadata(metadata: dict, path: Path) -> dict:
    """Ensure metadata contains source path and source reference fields."""
...
def _load_pdf(path: Path) -> str:
    """Extract text from a PDF file using fitz."""
    pages: List[str] = []
