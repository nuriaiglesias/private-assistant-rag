...
El primer resultado destacable es que el \textbf{BERTScore F1 es muy estable en todas las variantes} (0.563–0.575), lo que indica que la calidad semántica de las respuestas generadas no varía significativamente entre configuraciones. Esta estabilidad es coherente con la naturaleza de XQuAD: el corpus es reducido (240 pasajes únicos) y cualquier variante que recupere el pasaje correcto —algo que ocurre con alta probabilidad dadas las métricas de recuperación— genera una respuesta semánticamente equivalente.

La cobertura factual y ROUGE-L muestran un patrón diferente al observado en la evaluación interna. A y B obtienen la misma Fact-Coverage (0.280), mientras que C desciende a 0.200 y D a 0.160. Esto se debe a la interacción entre la segmentación del corpus y la naturaleza extractiva de los \textit{expected facts}: los 240 pasajes XQuAD se fragmentan en 502 chunks durante la ingesta, y el reranker de la variante C selecciona sub-fragmentos semánticamente relevantes que pueden no contener el \textit{span} exacto de la respuesta esperada. La métrica de solapamiento de tokens penaliza esta selección aunque el contenido recuperado sea factualmente correcto. En la variante D, el orquestador ReAct genera además respuestas en lenguaje natural —más completas y elaboradas— que divergen léxicamente de los \textit{spans} cortos y precisos que constituyen los \textit{expected facts} de XQuAD.

BERTScore, al medir similitud semántica mediante representaciones contextuales en lugar de solapamiento exacto de tokens, es la métrica más adecuada en este contexto. Los valores obtenidos (0.563–0.575) confirman que todas las variantes generan respuestas de calidad semántica equivalente, y que las diferencias en Fact-Coverage son un artefacto de la métrica y no una degradación real de la calidad de respuesta.
...
Finalmente, la evaluación sobre XQuAD-es refuerza las conclusiones anteriores desde una perspectiva de generalización. El ordenamiento de variantes se reproduce fielmente en un corpus de dominio diferente: C sigue siendo la configuración de recuperación más robusta (Hit@1 = 0.984, MRR = 0.991), y B continúa mostrando el mismo patrón de mayor diversidad en el top-3 a costa de precisión en la primera posición. En cuanto a la calidad de respuesta, el BERTScore es muy estable en todas las variantes (0.563–0.575), lo que confirma que la calidad semántica de las respuestas generadas es equivalente independientemente del método de recuperación empleado. Las diferencias en Fact-Coverage entre variantes en XQuAD son un artefacto de la naturaleza extractiva de los \textit{expected facts} del benchmark y de la segmentación del corpus en chunks, no una diferencia real de calidad de respuesta. BERTScore, al capturar similitud semántica en lugar de solapamiento léxico, es la métrica más adecuada para comparar el rendimiento del sistema entre ambas evaluaciones.
...
La evaluación complementaria sobre XQuAD-es (1.190 preguntas, 240 contextos únicos) confirma que el comportamiento del sistema se generaliza más allá del corpus de UNIR. Los resultados de recuperación reproducen el mismo ordenamiento observado en la evaluación interna —C $>$ A $>$ B en Hit@1 y C $>$ B $>$ A en Hit@3— con MRR de 0.991 para la variante C sobre un corpus de dominio distinto. En cuanto a la calidad de respuesta, el BERTScore es estable en todas las variantes (0.563–0.575), lo que indica que todas generan respuestas semánticamente equivalentes cuando el corpus es pequeño y bien alineado con las preguntas. Esta consistencia valida que la arquitectura no está sobreajustada al dominio institucional y que sus componentes muestran un comportamiento coherente y generalizable.