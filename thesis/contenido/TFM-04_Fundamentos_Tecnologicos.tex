...
El orquestador implementado sigue el paradigma ReAct \cite{yao2023react} (\textit{Reasoning + Acting}).
En cada iteración del bucle, el LLM produce un bloque estructurado con un pensamiento (\textit{Pensamiento}), la herramienta seleccionada (\textit{Herramienta}) y su argumento (\textit{Entrada}).
El sistema ejecuta la herramienta y devuelve la observación al modelo, que la incorpora en el siguiente paso.
Este ciclo \textit{Pensamiento $\to$ Acción $\to$ Observación} permite al orquestador razonar sobre la evidencia recuperada antes de decidir el siguiente paso, en lugar de seguir una secuencia fija predefinida.

El conjunto de herramientas disponibles delimita el ámbito de actuación del asistente.
La herramienta \texttt{buscar} permite recuperar fragmentos relevantes del corpus para una consulta concreta; puede invocarse varias veces con distintas reformulaciones.
La herramienta \texttt{rechazar} señala que no existe evidencia suficiente para responder con rigor.
La herramienta \texttt{redactar\_correo} genera un borrador de correo formal usando el contexto recuperado.
La herramienta \texttt{responder} indica que el LLM considera que ya dispone de evidencia suficiente y delega la generación de la respuesta final al componente RAG base.
...
En lugar de generar siempre una respuesta del mismo modo, el asistente adapta el flujo a la naturaleza de la consulta, y cada paso del razonamiento queda registrado como una llamada a herramienta observable.
Esto permite evaluar de forma independiente el impacto del orquestador sobre dimensiones como la abstención, la selección de herramientas y la estructuración de respuestas.
...
En el prototipo presentado se implementa una variante de \textit{query rewriting} activable mediante un parámetro de configuración.
