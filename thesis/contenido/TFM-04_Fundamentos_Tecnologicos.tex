...
Los ficheros de texto plano y Markdown se procesan directamente. 
Este flujo permite transformar distintas fuentes en una representación común, facilitando las etapas posteriores de limpieza, segmentación e indexación.
...
En este proyecto, la segmentación se plantea intentando preservar la coherencia semántica de los textos, combinando criterios como párrafos, oraciones y solapamiento entre fragmentos. 
...
Para la generación de embeddings se ha seleccionado BGE-M3, integrado mediante la librería \texttt{SentenceTransformers}. 
Este modelo ha sido diseñado para tareas de recuperación multilingüe y destaca por su enfoque \textit{multi-lingual}, \textit{multi-functionality} y \textit{multi-granularity}, lo que permite aplicarlo en distintos escenarios de búsqueda semántica \cite{chen2024bgem3}.

Resulta adecuado para un corpus en español y para posibles consultas formuladas en lenguaje natural y puede ejecutarse en un entorno local, lo que contribuye a mantener el control sobre los datos procesados. 
Por último, ofrece un equilibrio razonable entre calidad de recuperación y coste operativo, evitando la dependencia inicial de servicios externos de embeddings.
...
Además de la recuperación densa, el sistema contempla una variante híbrida que combina la similitud vectorial con BM25, un método clásico de recuperación léxica basado en coincidencia de términos y ponderación estadística \cite{robertson2009bm25}. 
Esta combinación permite reforzar consultas en las que determinados términos concretos, como nombres de trámites, titulaciones o conceptos administrativos, son especialmente relevantes.
...
Esta decisión permite mantener flexibilidad experimental: los modelos locales favorecen el control del entorno y la reducción de costes, mientras que las APIs externas pueden emplearse en fases concretas si se requiere una mayor calidad de generación. 
...
En el prototipo se contempla el uso de \textit{BAAI/bge-reranker-v2-m3} como modelo principal de reranking, manteniendo la posibilidad de probar modelos más ligeros en la evaluación experimental. 
Esta separación permite comparar el impacto del reranking sobre la calidad de las respuestas sin modificar el resto del pipeline RAG.
...
Su principal ventaja en este proyecto es que permite construir una interfaz funcional con un esfuerzo reducido, sin introducir una capa de desarrollo frontend compleja.
