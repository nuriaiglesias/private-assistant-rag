...
Los artículos más recientes han explorado con especial interés las arquitecturas de agentes y los mecanismos de orquestación, el uso de herramientas y APIs externas, los sistemas de \textit{Retrieval-Augmented Generation} (RAG) y, más recientemente, los asistentes especializados para dominios institucionales o cerrados. 
...
Además, el trabajo incorpora una contribución experimental específica centrada en la mejora del componente RAG mediante una estrategia de recuperación híbrida y \textit{reranking}. 
...
En esta dirección, Yao et al.\ proponen ReAct, un enfoque que combina trazas de razonamiento con acciones explícitas ejecutadas sobre fuentes externas o entornos interactivos \cite{yao2023react}. 
...
Sobre esta idea inicial, la investigación posterior ha ampliado el foco desde agentes individuales hacia sistemas compuestos por múltiples agentes especializados, como AgentVerse, un marco de colaboración multiagente en el que distintos agentes cooperan para resolver tareas \cite{chen2024agentverse}.
...
Hong et al.\ proponen MetaGPT \parencite{hong2024metagpt} que frente a aproximaciones más informales, MetaGPT introduce una organización más estructurada del trabajo entre agentes, de forma que cada componente participa en una fase concreta del proceso y verifica resultados intermedios. 
...
Por su parte, Wu et al.\ presentan AutoGen \cite{wu2024autogen}, uno de los elementos más destacables es que los agentes pueden operar en distintos modos, combinando modelos de lenguaje, intervención humana y uso de herramientas externas. 
...
Más recientemente, Dang et al.\ plantean un esquema centralizado en el que un orquestador dirige de forma adaptativa a los distintos agentes según el estado de la tarea \cite{dang2025evolving}. 
...
Uno de los trabajos de referencia en esta línea es Toolformer, propuesto por Schick et al.\ \cite{schick2023toolformer}. 
...
En este sentido, API-Bank, de Li et al.\ \cite{li2023apibank}, presentan un \textit{benchmark} en el que se evalúan aspectos como la planificación, la recuperación de la API correcta y la ejecución de llamadas. 
...
En este contexto, Lewis et al.\ introducen RAG como un marco que combina un modelo generativo con una memoria externa, accesible mediante un mecanismo de recuperación \cite{lewis2020rag}. 
Establece una base clara para integrar conocimiento externo durante la generación, mejorando la especificidad, la factualidad y la trazabilidad de las respuestas. 
Este trabajo es el punto de partida para justificar el uso de un sistema RAG como componente central de consulta.
...
En esta línea, Shi et al.\ proponen REPLUG, donde tratan al modelo de lenguaje como una caja negra y utilizan un recuperador ajustable cuyos documentos se añaden directamente al \textit{prompt} del modelo \cite{shi2024replug}.
Demostraron que os beneficios de RAG no dependen necesariamente de modificar la arquitectura del modelo, sino que pueden alcanzarse mediante mecanismos externos de recuperación e integración de contexto. 

Asai et al.\ presentan Self-RAG, un enfoque en el que el modelo aprende no solo a recuperar información, sino también a reflexionar sobre la necesidad de dicha recuperación y a criticar la calidad de la respuesta generada \cite{asai2024selfrag}. 
Esta contribución desplaza de la idea de un RAG estático hacia uno más adaptativo y autorreflexivo. 

Jeong et al.\ proponen Adaptive-RAG, una arquitectura que selecciona dinámicamente la estrategia de recuperación más adecuada en función de la complejidad de la pregunta \cite{jeong2024adaptiverag}. 
...
En este sentido, Zeng et al.\ estudian cómo un sistema RAG puede exponer contenido de la base de recuperación y evidencian que el uso de información privada no solo aporta beneficios en términos de utilidad, sino que también exige nuevas medidas de protección \cite{zeng2024ragprivacy}. 
...
Un punto de partida fundamental en esta línea es el trabajo de Karpukhin et al.\ sobre \textit{Dense Passage Retrieval} (DPR) \cite{karpukhin2020dpr}. 
...
Entre ellos, RocketQA, propuesto por Qu et al.\ \cite{qu2021rocketqa}, introduce mejoras específicas como la selección de negativos poco informativos o la existencia de falsos negativos en el entrenamiento. 
...
Nogueira et al.\ demuestran que un modelo generativo puede emplearse eficazmente para calcular relevancia documental y reordenar documentos previamente recuperados \cite{nogueira2020seq2seqranking}. 
...
En este punto cobra especial relevancia el trabajo de Saad-Falcon et al.\ sobre ARES \cite{saadfalcon2024ares}, el cual propone evaluar separadamente distintas dimensiones del sistema, como la relevancia del contexto recuperado y la relevancia global de la respuesta generada. 
...
En el ámbito universitario, Trienes et al.\ presentan Marcel, un agente conversacional ligero y de código abierto orientado al soporte de estudiantes \cite{trienes2025marcel}, su alcance permanece centrado en la respuesta a consultas de admisión.
...
Una propuesta más robusta en escenarios cerrados es URASys, de Nguyen et al., que plantea un sistema unificado para responder preguntas estándar y ambiguas en tareas de \textit{academic advising} \cite{nguyen2025urasys}.
...
Más allá del entorno universitario, encontramos varias propuestas. Liu et al.\ proponen WorkTeam, un marco multiagente para transformar instrucciones en lenguaje natural en flujos de trabajo estructurados en entornos empresariales \cite{liu2025workteam}. 
...
En una línea cercana al uso de herramientas, Zhang et al.\ presentan AskToAct \cite{zhang2025asktoact}. 
...
Por último, Kalra et al.\ presentan HyPA-RAG, un sistema de \textit{Retrieval-Augmented Generation} adaptativo para aplicaciones legales \cite{kalra2025hyparag}. 
...
La propuesta de este TFM se sitúa en la intersección entre asistentes institucionales especializados, sistemas de uso de herramientas y mejora experimental de \textit{Retrieval-Augmented Generation}. 
...
Finalmente incorpora una contribución experimental específica sobre el componente RAG basada en recuperación híbrida y \textit{reranking}.
...
Finalmente, HyPA-RAG constituye el antecedente más cercano en la parte investigadora, ya que también propone mejorar el componente RAG mediante recuperación híbrida y ajuste adaptativo. 
...
En conjunto, los trabajos revisados muestran la existencia de asistentes especializados en dominios institucionales o cerrados, el interés creciente por la coordinación de herramientas, y la mejora del propio pipeline RAG como vía activa de investigación aplicada. 
Sin embargo, estas líneas suelen aparecer separadas y este vacío justifica el interés de una propuesta que combine consulta documental fundamentada, uso de herramientas orientadas a la acción y mejora experimental del componente de recuperación dentro de un mismo asistente institucional.
